...
export function SuperprofileStudio() {
  ...
  const [courseCheckoutUrl, setCourseCheckoutUrl] = useState("https://topmate.io");
  ...
  const [sessionUrl, setSessionUrl] = useState("https://topmate.io");
  ...
  const [prodUrl, setProdUrl] = useState("https://gumroad.com");
  ...
  async function handleAddCourse() {
    if (!courseTitle.trim() || !courseCheckoutUrl.trim()) {
      toast.error("Course title aur checkout URL zaroori hain");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Course add nahi ho saka");
      toast.success(`Course "${courseTitle}" bio me add ho gaya!`);
    } catch (err: any) {
  ...
  async function handleAddSession() {
    if (!sessionTitle.trim() || !sessionUrl.trim()) {
      toast.error("Session title aur booking link zaroori hain");
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Session add nahi ho saka");
      toast.success(`1:1 Session card bio me add ho gaya!`);
    } catch (err: any) {
      toast.error(err.message);
    } finally {
  ...
  async function handleAddProduct() {
    if (!prodTitle.trim() || !prodUrl.trim()) {
      toast.error("Product title aur URL zaroori hain");
      return;
    }
    setSavingProd(true);
    try {
      ...
      toast.success(`Digital product bio me add ho gaya!`);
    } catch (err: any) {
      toast.error(err.message);
    } finally {
      setSavingProd(false);
  ...
  async function handleAddUpi() {
    if (!upiId.trim()) {
      toast.error("Valid UPI ID likhein");
      return;
    }
    setSavingTip(true);
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "UPI link add nahi ho saka");
      toast.success("UPI payment card bio me add ho gaya!");
    } catch (err: any) {
      toast.error(err.message);
    } finally {
      setSavingTip(false);
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-400">Superprofile-grade course selling with multi-chapter syllabus & lock previews.</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              ...
              <input
                ...
                placeholder="https://topmate.io/... or https://gumroad.com/l/..."
                className="mt-1 h-10 w-full rounded-xl border border-white/10 bg-black/40 px-3 text-sm text-white focus:border-violet-500/50 focus:outline-none"
              />
            </div>
            <div>
      ...
      {activeTab === "sessions" && (
        <div className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
          ...
          <div>
            <label className="text-[11px] font-medium text-zinc-400">Booking Link (Topmate / Cal.com / Calendly)</label>
            <input
              value={sessionUrl}
              onChange={(e) => setSessionUrl(e.target.value)}
              placeholder="https://topmate.io/your_handle/..."
              className="mt-1 h-10 w-full rounded-xl border border-white/10 bg-black/40 px-3 text-sm text-white focus:border-sky-500/50 focus:outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              ...
              <input
                ...
                placeholder="https://gumroad.com/l/... or https://payhip.com/..."
                className="mt-1 h-10 w-full rounded-xl border border-white/10 bg-black/40 px-3 text-sm text-white focus:border-fuchsia-500/50 focus:outline-none"
              />
            </div>
            <div>
        ...
        <div className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-base font-bold text-white">Direct UPI & Creator Tipping</h2>
                <p className="text-xs text-zinc-400">Accept 0% commission direct support via Indian UPI apps (Google Pay, PhonePe, Paytm).</p>
              </div>
            </div>
